  ...
  //  const { products, error } = useProducts();
	...
	watchEffect(() => {
	  if (uniqueCountries.value.length && selectedCountries.value.length === 0) {
		...
		const fuse = computed(() => {
		  return new Fuse(products.value, {
			keys: ['model', 'manufacturer', 'key'],
			threshold: 0.3
		  })
		})
    ...
    const finalFilteredProducts = computed(() => {
	 ...
	 const homepageSearchTerm = route.query.search || ''
	 console.log('homepageSearchTerm-'+homepageSearchTerm)
		  // If there's a search query  match by model, key, or manufacturer
		  if (homepageSearchTerm.length > 0) {
		  	 if (!homepageSearchTerm) return transformedData.value
			  return fuse.value.search(homepageSearchTerm).map(r => r.item)
		  }else{
